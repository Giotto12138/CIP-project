    ...
    if(scanned)
    {
        if(scanned->priority_count >= conf->priority_count)
        {
            ...
            if ((action == RULE_TYPE__DROP) ||
                ...
                (!scanned->alerts))
            {   
                // if the number of ip addresses communicating with scanned machine < 100
                // and the number of ports communicating with scanned machine >= 10
        ...
        PS_ALERT_CONF *conf, int proto)
{
    ...
    if(scanner)
    {
        ...
        // if scanner's active connections on the host (SRC or DST) in the current time period >= 30
        ...
        {   
            ...
            if ((action == RULE_TYPE__DROP) ||
                ...
                (!scanner->alerts))
            {   
                // if scanner's ip addresses >= 3 and scanner's port number <= 10
                if(scanner->u_ip_count >= conf->u_ip_count &&
                   scanner->u_port_count <= conf->u_port_count)
                {   
                    // filtered portsweep is triggered
...
static int ps_alert_tcp(PS_PROTO *scanner, PS_PROTO *scanned)
{
    ...
    */
    ...
    if((portscan_eval_config->detect_scan_type & PS_TYPE_DECOYSCAN) &&
        ps_alert_one_to_one_decoy(scanner, scanned, one_to_one_decoy, PS_PROTO_))
    {
        return 0;
    }

    if((portscan_eval_config->detect_scan_type & PS_TYPE_PORTSWEEP) &&
        ps_alert_one_to_many(scanner, scanned, one_to_many, PS_PROTO_))
    {
        return 0;
    }
